...
const Todo: React.FC<TodoProps> = ({ org, repo }) => {
  ...
  const [generateStates, setGenerateStates] = useState<Record<number, boolean>>(
    {},
  );
  ...
  const handleGenerateResearch = async (todoId: number) => {
    setGenerateStates((prev) => ({ ...prev, [todoId]: true }));
    try {
      await trpcClient.todos.researchIssue.mutate({ todoId, org, repo });
    } catch (error) {
      console.error("Error generating research:", error);
    } finally {
      setGenerateStates((prev) => ({ ...prev, [todoId]: false }));
    }
  };
          ...
          {isLoadingTodos ? (
            ...
            filteredTodos.map((todo, index) => (
              <TodoItem
                ...
                isGenerating={!!generateStates[todo.id]}
      ...
      <div className=" hide-scrollbar h-[calc(100vh-116px)] w-2/3 overflow-y-scroll bg-white p-6 dark:bg-gray-800 ">
        {selectedTodo ? (
          <IssueDetails
            ...
            org={org}
            repo={repo}
            isGenerating={!!generateStates[selectedTodo.id]}
