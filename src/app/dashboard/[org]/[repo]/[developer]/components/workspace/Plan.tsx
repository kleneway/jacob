  ...
  <div className="w-full bg-blueGray-900 p-2 pt-0 text-gray-100">
    ...
    <div className="grid w-full grid-cols-1 gap-4 p-2 md:grid-cols-2 lg:grid-cols-3">
      {planSteps.map((plan, idx) => {
        const isCurrentStep = !(plan as Plan).isComplete && idx === currentPlanStep;
        return (
          <div
            key={(plan as Plan).id}
            className={`relative max-w-sm transform rounded-lg p-4 shadow-lg transition-all duration-300 ease-in-out hover:scale-105 ${
              idx === currentPlanStep
                ...
                : "bg-blueGray-800"
            } ${(plan as Plan).isComplete ? "opacity-70" : "opacity-100"}`}
          >
            <header className={`flex items-center justify-between text-white`}>
              <h3
                className={`font-semibold ${isCurrentStep ? "text-orange-400" : ""} ${(plan as Plan).isComplete && !isCurrentStep ? "line-through opacity-60" : ""}`}
              >
                {idx + 1}. {(plan as Plan).title}
              </h3>
              <FontAwesomeIcon
                icon={
                  isCurrentStep
                    ? faCircle
                    : (plan as Plan).isComplete
                      ? faCheckCircle
                      : faCircleDot
                }
                className={`h-3 w-3 text-xl ${isCurrentStep ? "animate-pulse text-orange" : (plan as Plan).isComplete ? "text-light-blue" : "rounded-full border-2 border-blueGray-500 text-transparent"}`}
              />
            </header>
            <div className="mt-2 text-gray-300">
              <p>{(plan as Plan).description}</p>
            </div>
            {isCurrentStep && (
